...
Client::connectToTracker(string tracker_ip, int port) {
	...
	*/
	if (tracker_ip != ip_address) {
		trackerfd = connectToHost(tracker_ip, port);
	} else {
		trackerfd = FIRST_NODE;
	}
}
...
Client::registerWithTracker() {
	...
	if (trackerfd == FIRST_NODE) {
		// The initiator adds itself to its tracker
		Client tmp;
		tmp.deserialize(data, size);
		t.client_register(tmp);
	} else {

		/*
		 * Register with the node which is already member so that this client is
		 * added to that tracker's information.
		 */
		memcpy(header, (char *)&op, sizeof(int));
...
Client::updateOnTracker() {
	...
	for (int i = 0; i < peers.size(); i++) {
		if (peers[i].getIP() == ip_address && peers[i].getPort() == port) {
			//cout<<"Self - ignore."<<endl;
		} else {
			// Register with every peer
			connectToTracker(peers[i].getIP(), tracker_port);
			sockfd = trackerfd;
			// now trackerfd corresponds to this peer
			// this client now updates the tracker on this peer
			/*
			   int bytes_sent = send(sockfd, header, HEADER_SZ, 0);
			   assert(bytes_sent == HEADER_SZ);
			 */
...
//Only done when the client joins the network for the first time
void
Client::queryTracker() {
	char header[HEADER_SZ];
	...
	for (int i = 0; i < num_clients; i++) {
		...
		if (c.getIP() == ip_address && c.getPort() == port) {
			...
			if (trackerfd != FIRST_NODE) {
				// Register with every peer
				connectToTracker(c.getIP(), tracker_port);
				// now trackerfd corresponds to this peer
				// this client now registers with the tracker on this peer
				registerWithTracker();
				close(trackerfd);
			}
		}
